...
TOTAL_POPULARITY = sum(map(sum, POPULARITY))
    ...
    for r in range(N):
        for c in range(N):
            if region[r][c] == 5:
                ...
                if r < x + d1 and c <= y:
                    sum_list[0] += POPULARITY[r][c]
                elif r <= x + d2 and c > y:
                    sum_list[1] += POPULARITY[r][c]
                elif r >= x + d1 and c < y - d1 + d2:
                    sum_list[2] += POPULARITY[r][c]
                elif r > x + d2 and c >= y - d1 + d2:
                    sum_list[3] += POPULARITY[r][c]
