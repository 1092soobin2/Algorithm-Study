# (2) 구현__BOJ_19237_어른상어
# 1. 분산된 정보 함께 업데이트 / 2. 단순 시뮬은 재귀가 아니므로 copy 인자로 전달 필요 x /
...
shark_directions = list(map(map_direction, map(int, input().split())))
...
for _ in range(M):
    ...
    for _ in range(4):
        # up, down, left, right
        tmp_list.append(list(map(map_direction, map(int, input().split()))))
    tmp_list2 = [tmp_list[0], tmp_list[2], tmp_list[1], tmp_list[3]]
    priority_list.append(tmp_list2)
    ...
    for r in range(N):
        for c in range(N):
            if s_board[r][c] != 0:
                ...
                sharks.append([shark_id, (r, c), s_dlist[shark_id-1]])
...
def update_scent_board(moved_shark_list):
    ...
    # 2) 움직인 상어들 냄새 업데이트
    copied_list = copy.deepcopy(moved_shark_list)
    for shark in copied_list:
        s_id, (r, c), _ = shark
...
ans = 0
while ans <= 1000 and len(sharks) != 1:
    move_sharks(sharks)
